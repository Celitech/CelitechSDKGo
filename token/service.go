...
// Service provides methods to interact with Token-related API endpoints.
// It uses a configuration manager for settings and supports custom hooks for request/response interception.
type Service struct {
	manager             *configmanager.ConfigManager
	hook                hooks.Hook
	generateTokenConfig []celitechconfig.RequestOption
}
...
// SetGenerateTokenConfig sets method-level configuration for GenerateToken.
// Options are applied to every future call to GenerateToken and take
// precedence over service-level config. Per-call options still take highest precedence.
func (api *Service) SetGenerateTokenConfig(opts ...celitechconfig.RequestOption) *Service {
	api.generateTokenConfig = opts
	return api
}

// Generate a new token to be used in the iFrame
func (api *Service) GenerateToken(ctx context.Context, params GenerateTokenRequestParams, opts ...celitechconfig.RequestOption) ([]byte, error) {
	config := *api.config()
	for _, opt := range api.generateTokenConfig {
		opt(&config)
	}
