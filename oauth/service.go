...
func (api *Service) GetAccessToken(ctx context.Context, oAuthTokenRequest OAuthTokenRequest, opts ...celitechconfig.RequestOption) (*OAuthTokenResponse, error) {
	config := *api.config()
...
func (api *Service) GetOAuthAccessToken(config celitechconfig.Config, scopes []string) (oauthtokenmanager.GetOAuthAccessTokenResponse, error) {
	if config.ClientID == nil {
		return nil, fmt.Errorf("oauth config field ClientID is required")
	}
	if config.ClientSecret == nil {
		return nil, fmt.Errorf("oauth config field ClientSecret is required")
	}
	...
	var expiresIn *int64
	if n := response.ExpiresIn; n != nil && !n.IsNull {
		expiresIn = &n.Value
	}
	return oauthtokenmanager.NewOAuthTokenResponse(response.AccessToken, expiresIn), nil
}
